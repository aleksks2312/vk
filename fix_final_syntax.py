...
script_content = """<script>
        ...
        function previewBg(reqLevel, urlName, title) {
            ...
            if(altarLevel < reqLevel) {
                btn.innerText = `🔒 Откроется на ${reqLevel} ур.`;
                btn.disabled = true;
                btn.onclick = null;
            } else {
                btn.innerText = "Выбрать этот фон";
                btn.disabled = false;
                btn.onclick = () => {
                    activeBgUrl = urlName;
        ...
        function checkTutorial() {
            ...
            if(activeTab === 'divination' && !safeStorageGet('tut_divination')) {
                showTutDialog(
                    "Голос Вселенной", 
            ...
            else if(activeTab === 'rituals' && !safeStorageGet('tut_rituals')) {
                showTutDialog(
                    "Жертва Магии", 
        ...
        function replayTutorial() {
            safeStorageRemove('tut_divination');
            safeStorageRemove('tut_altar');
            safeStorageRemove('tut_grimoire');
            safeStorageRemove('tut_rituals');
            safeStorageRemove('tut_draw_done');
            energy = 1;
            updateStatsUI();
